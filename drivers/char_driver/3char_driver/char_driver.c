/************************************************************************************************
@ileName : char_driver.c
@author	 : Mahesh Terkar 
@teamLead: Rajesh Dommaraju 
@etails  : It is a char  driver to load a module in the kernel  
...
#include<linux/uaccess.h>//cpy from usr ,cpy to user
...
#define DRIVER_MODULE_DESC			"char driver Module"
...
LOCAL int etx_open(struct inode *inode,struct file *file)
{
	if(!(kern_buff = kmalloc(mem_size,GFP_KERNEL)))
	{	
		printk(KERN_INFO "---->ERROR: Can Not allocate memory in kernel...\n");
		return -1;
	}
...
LOCAL ssize_t etx_read(struct file *filp, char __user *buf, size_t len, loff_t *off)
{
	copy_to_user(buf,kern_buff,mem_size);
	printk(KERN_INFO "---->Data read Complited...\n");
	return mem_size;
}
...
LOCAL ssize_t etx_write(struct file *filp, const char *buf, size_t len, loff_t *off)
{
	copy_from_user(kern_buff,buf,len);
	printk(KERN_INFO "---->Data write complited\n");
	return len;
